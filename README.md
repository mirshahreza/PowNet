...
- [Localization (FA / ?????)](#persian-summary-?????-?????)
- [License](#license)
...
PowNet
 ...
 ??? Tests (395 scenarios validating behaviour)
```
...
`.NET extensions`, `.NET utility toolkit`, `C# performance helpers`, `DbIO abstraction`, `ADO.NET wrapper`, `event bus C#`, `configuration overrides .NET`, `diagnostic extensions`, `code quality analysis .NET`, `development utilities`, `testable infrastructure`, `PowNet framework`.

---
## Requirements
- .NET 10 (preview-compatible; update as stable RTMs)
